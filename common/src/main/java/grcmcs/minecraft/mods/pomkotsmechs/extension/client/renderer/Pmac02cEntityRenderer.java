...
import grcmcs.minecraft.mods.pomkotsmechs.extension.client.model.Pmac02cEntityModel;
...
public class Pmac02cEntityRenderer extends GeoEntityRenderer<Pmac02cEntity> {
    public Pmac02cEntityRenderer(EntityRendererProvider.Context renderManager) {
        this(renderManager, new Pmac02cEntityModel());
    }

    public Pmac02cEntityRenderer(EntityRendererProvider.Context renderManager, Pmac02cEntityModel model) {
        super(renderManager, model);
        addRenderLayer(new AutoGlowingGeoLayer<>(this));
    }

    @Override
    public void preApplyRenderLayers(PoseStack poseStack, Pmac02cEntity animatable, BakedGeoModel model, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
        this.scaleHeight = Pmac02cEntity.DEFAULT_SCALE;
        this.scaleWidth = Pmac02cEntity.DEFAULT_SCALE;
    ...
    @Override
    public void actuallyRender(PoseStack poseStack, Pmac02cEntity animatable, BakedGeoModel model, RenderType renderType,
        ...
        if (animatable.getDrivingPassenger() != null) {
            animatable.setClientSeatPos(getSeatPosition(model, animatable, Pmac02cEntity.DEFAULT_SCALE));
            animatable.setMainCameraPosition(MSRenderUtils.getMainCameraPosition(model, animatable, partialTick, Pmac02cEntity.DEFAULT_SCALE));
        }
